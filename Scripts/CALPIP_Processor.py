...
# FUNCTIONS
####################################################################
def writeApplicationData(df, year, cropAlias, chemAlias):
    #remapping column names that may differ year-by-year to universal names
    headers = {"acre_unit_treated": "acreage",
                "acre_planted": "acreage",
                "chemical_code": "chemCode",
                "chemical_no": "chemCode",
                "chem_code" : "chemCode",
                "commodity_code": "cropID",
                "site_code": "cropID",
                "township" : "township",
                "tship_dir" : "tship_dir",
                "range" : "range",
                ...
                "section" : "section",
                "applic_dt": "date"}
    columns = ["acreage", "chemCode", "cropID", "TRS", "date"] #can add more here
    df.rename(index=str, columns=headers, inplace=True)
    df["TRS"] = df["township"]+df["tship_dir"]+df["range"]+df["range_dir"]+df["section"]
    df = df[columns].copy()

    #removing rows for which we are missing some data
    df.dropna(inplace=True) # drops all rows with missing values, can adjust later to only drop if missing certain fields

    #removing rows with crops or chemicals that are not in our lookup 
    df['cropID'] = df['cropID'].astype(int)
    df['chemCode'] = df['chemCode'].astype(int)
    df['chemCode'].fillna(-999)
    df = df[df['cropID'].isin(cropAlias.keys())]
    df = df[df['chemCode'].isin(chemAlias.keys())]

    #TODO: print out if crops / chemcials are not in keys() so we can add them later

    #extracting crop names, chemical names, and date
    df['cropName'] = df['cropID'].map(lambda cropID: cropAlias[cropID][1])
    df['cropGroup'] = df['cropID'].map(lambda cropID: cropAlias[cropID][2])
    df['chemName'] = df['chemCode'].map(lambda chemCode: chemAlias[chemCode][1])
    f = lambda date: extractDayMonthYear(parseCalpipDate(date, year), '%m/%d/%Y')
    df['day'], df['month'], df['year'] = zip(*df.date.map(f))
    # df.apply(func = (lambda row: extractDayMonthYear(parseCalpipDate(row.date, year), '%m/%d/%Y')), axis=1, result_type='expand')
    if year < 1990:
        df['acreage'] = df['acreage'].astype(float) / 100
        #put other time-specific logic here
    elif year >= 1990:
        df['acreage'] = df['acreage'].astype(float)
        # df['date'].apply()
    # df.merge(df.date.apply(f, result_type='expand'))

    #add checkstr column

    #add filtered column initally set to false

    #see what percent of the filtereing is each crop responsible for

    #what percent of each crop is filtered

    # gb = df.groupby(['TRS'])
    # print(gb.get_group('14S03E08').groupby(["month", "cropName"]).head())
    # print(gb.get_group('14S03E08').groupby(["month", "cropName"]).describe())
    gb = df.groupby(['TRS','month','cropName'])
    print(gb.get_group(('14S03E08', 1, 'Artichokes')))
    # with pd.option_context('display.max_rows', None, 'display.max_columns', None):  # more options can be specified also
        # print(df.groupby(['TRS', 'month', 'cropName']).get_group(['14S03E08'])['acreage'].mean())
...
def aggregateCropData(df, year):
    ...
    for index, R in df.iterrows():
        ...
        if  pd.isnull(R.applic_dt):
            ...
            date=parseCalpipDate(R.applic_dt, year)
        try:
...
def filterCropDict(cropDict, year):
    ...
    with open ( file, 'w') as fout:
        ...
        fout.write('\n')
        for value in range (0, len(test)):
            for item in range (0,len(test[value][2])):
...
def filterRanchDict(ranchDict):
    ...
    fmt='%m/%d/%Y'
    for ranch in list(ranchDict.keys()):
        #ranchDict[ranchID][0][month][0].append(cropID)
        #ranchDict[ranchID][0][month][1].append(acreage)
        #ranchDict[ranchID][0][month][2].append(date)
        astor=list()
...
def createRanchDict(ranchDict, ranchID):
    ...
    monthCrop=dict()
    regionID=0
    if ranchID not in list(ranchDict.keys()):
...
def createTRSDict(TRSLU,TRS):
    ...
    if TRS not in list(TRSLU.keys()):
        ...
        for key in list(monthCrop.keys()):
            skipPeriod=False
            for cropIDX in list(cropDict.keys()):
...
def aggregateByTRS(year, cropDict, cropAlias, selectedCropGroups, annualCrops, outFile1, outFile2, outFile3, outFile4):
    ...
    for TRS in TRSLU:
        ...
        for month in range(1,13):
            ...
            for group in selectedCropGroups:
                if group in annualCrops:
                    if month==1: #check that crop does not exist for entire year
                        ...
                        for monthcheck in range(1,13):
                            if group in list(TRSLU[TRS][0][monthcheck].keys()) and TRSLU[TRS][0][monthcheck][group]['area'] > 0.:
                ...
                elif group in list(TRSLU[TRS][0][month].keys()):
    ...
    for TRS in TRSLU:
        for period in periods:
            sumAcres=0.
            plantedGroups=[x for x in list(TRSLU[TRS][0][period].keys()) if TRSLU[TRS][0][period][x]['area'] > 0]
        ...
        for TRS in TRSLU:
            ...
            for month in range (1,13):
                ...
                if writeGroup:
                    ...
                    for cropIDX in list(TRSLU[TRS][0][month].keys()):
    ...
    with open (outFile3, 'w') as fout: #%total area
        for TRS in TRSLU:
            ...
            groupList=[]
            if writeGroup:
                for group in list(TRSLU[TRS][0]['total'].keys()):
                ...
                for cropIDX in list(TRSLU[TRS][0]['total'].keys()): #compute total sum
                    sumTotal+=TRSLU[TRS][0]['total'][cropIDX]['area']
                for cropIDX in list(TRSLU[TRS][0]['total'].keys()):
                    if sumTotal > 0.0:
                        cropName=cropAlias[cropIDX][1]
                        fout.write('{0}\t'.format(cropName))
    ...
    for month in constants.CAF[1]:
        ...
        with open (outFile, 'w') as fout: #%total area
            ...
            for TRS in TRSLU:
                #oneGroup=False
                region=primaryregionLookup[TRS]
                if len(list(TRSLU[TRS][0]['total'].keys()))>0: #TRS has selected crop groups
                    fout.write ('{0}\t{1}\t'.format(region, TRS))
                    sumAcres=0.
                    if writeGroup: #cropIDx logic is not been tested in here...todo check that sum <=1.0
                        for group in selectedCropGroups:
                            if group in list(TRSLU[TRS][0][month].keys()) and TRSLU[TRS][0][month][group]['fraction']>0.:
                                fout.write('{0}\t'.format(group))
                                fout.write('{:.4f}\t'.format(TRSLU[TRS][0][month][group]['fraction']))
                    else:
                        for cropIDX in list(TRSLU[TRS][0][month].keys()):
...
def cropRotationFrequency(df):
    ...
    for region in regionList:
        ...
        for month in range(1,13): #determine names of ind crops/groups
            for key in list(monthDict[month].keys()):
        ...
        for crop in cropList:
            ...
            errors=list()
            for month in range(1,13):
                if crop in list(monthDict[month].keys()):
                            ...
                            for idx in range (0, len(ranchDict[ranch][0][month][0])):#for the number of crops in that month
                                ...
                                if printByGroup:
                                    if cropAlias[cropIDX][2] in selectedCropGroups:#print out crop group instead of crop name
                                       if cropIDX in list(tempDict.keys()):
                                       ...
                                       if idx==len(ranchDict[ranch][0][month][0])-1: #last time in loop
                                            for idx2 in list(tempDict.keys()):
                                                cropAdded=True
...
def updateCropColors(cropDict):
    if constants.firstTime:
        ...
        for cropID in sorted(cropDict.keys()):
            if cropID in list(constants.cropColors.keys()):
...
def computeregionStatistics(cropDict, statsFile):
    ...
    for cropIDX in list(cropDict.keys()):
        crop=cropDict[cropIDX]
        for region in list(crop.regions.keys()):
            climate=regionClimateLookup[region]
            if region not in list(regionStats.keys()):
                regionStats[region]=dict()
            if crop.LUCategory not in list(regionStats[region].keys()):
    ...
    with open(statsFile, append_write) as fout:
        ...
        if append_write=='w':
            for idx in range (0, len(headers)):
                fout.write('%s' % headers[idx]+'\t')
        for region in list(regionStats.keys()):
            for LU in list(regionStats[region].keys()):
...
def findregionByTRS(TRS):
    if TRS in list(primaryregionLookup.keys()):
        region=primaryregionLookup[TRS]
    return region
...
def readAlias(aliasFile1, aliasFile2):
    ...
    for cropIDX in list(cropAlias1990.keys()):
        groupName=cropAlias1990[cropIDX][2] 
        if groupName not in list(cropGroups.keys()):
            ...
            for idx in range(0, len(crop.regions[key][0])):
                ...
                if year>=1990:
                    ...
                    try:
                        ...
                        constants.checkFlag=True
                        print('Issue with acres in aggregate regions')
            for month in list(monthAcres.keys()):
...
def aggregateByregionGroup(cropDict, cropAlias, cropGroups, year):
    ...
    for regionID in range (1, 32):
        ...
        for group in list(cropGroups.keys()): # for each crop
            monthAcres=dict()
            times=list()
            values=list()
            for cropIDX in cropGroups[group]:
                ...
                if cropIDX not in cropGroups[group]:
                    continue
                if cropIDX not in list(cropDict.keys()):
                    continue
                try:
                    crop=cropDict[cropIDX]
                except:
                    print('test')
                if regionID not in list(crop.regions.keys()):
                    continue
                ...
                with z.open('site.txt') as f:
                    ...
                    for line in f:
                        if firstLine:
                            ...
                            continue
                        words = [tok.strip() for tok in line.split(",")]
                        if words[0] in list(lookupDict.keys()):
                            continue
                        else:
...
if __name__ == "main":
    ...
    for year in constants.times:
        ...
        # print(cropAlias1974.keys())
        # print(cropAlias1990.keys())
        ...
        writeApplicationData(df, year, cropAlias, chemAlias)
    ...
    #         fout.write ('{0}\t'.format(year))
    #         for group in selectedCropGroups:
    #             if group in list(groupStore[year].keys()):
